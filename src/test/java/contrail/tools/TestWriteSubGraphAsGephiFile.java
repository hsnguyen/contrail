 ...
 */
public class TestWriteSubGraphAsGephiFile {
  ...
  public void testGephiFile () {
    ...
    Path avroPath = new Path(
        FilenameUtils.concat(tempDir.getPath(), "indexed_graph"));
    Configuration conf = new Configuration();
    GraphUtil.writeGraphToIndexedFile(
        conf, avroPath,
        new ArrayList<GraphNode>(builder.getAllNodes().values()));

    WriteSubGraphAsGephiFile stage = new WriteSubGraphAsGephiFile();
    ...
    parameters.put("start_node", "ACTG");
    parameters.put("max_size", 3);
