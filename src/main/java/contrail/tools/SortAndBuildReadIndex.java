 ...
 *
 */
public class SortAndBuildReadIndex extends PipelineStage {
  private static final Logger sLogger =
      Logger.getLogger(SortAndBuildReadIndex.class);
  ...
  protected Map<String, ParameterDefinition> createParameterDefinitions() {
    ...
    StageBase[] substages = {new SortReads(), new CreateReadsIndex()};
  ...
  protected void stageMain() {
    ...
    // Sort the graph.
    SortReads sortStage = new SortReads();
    sortStage.initializeAsChild(this);
    sortStage.setParameter("inputpath", inputPath);

    String sortedReads = FilenameUtils.concat(outputPath, "sorted");

    sortStage.setParameter("outputpath", sortedReads);

    executeChild(sortStage);

    CreateReadsIndex indexStage = new CreateReadsIndex();
    indexStage.initializeAsChild(this);

    String indexPath = FilenameUtils.concat(outputPath, "indexed");

    indexStage.setParameter("inputpath", sortedReads);
    indexStage.setParameter("outputpath", indexPath);

    executeChild(indexStage);

    sLogger.info("Indexed reads written to:" + indexPath);
  }

  public static void main(String[] args) throws Exception {
    int res = ToolRunner.run(
        new Configuration(), new SortAndBuildReadIndex(), args);
    System.exit(res);
  }
}
